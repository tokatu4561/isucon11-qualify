...
// ISUからのコンディションを受け取る
// TODO: ここが cpu 使用大きい alp で sum が多い
func postIsuCondition(c echo.Context) error {
	...
	for _, cond := range req {
		...
		prepareStmtParams = append(prepareStmtParams, "(?, ?, ?, ?, ?)")
		values = append(values, jiaIsuUUID)
		values = append(values, timestamp)
		values = append(values, cond.IsSitting)
		values = append(values, cond.Condition)
		values = append(values, cond.Message)
	}

	sqlQuery := fmt.Sprintf("INSERT INTO `isu_condition` (`jia_isu_uuid`, `timestamp`, `is_sitting`, `condition`, `message`) VALUES %s", strings.Join(prepareStmtParams, ","))
